# filesystem-layout-lifeos-meta-front-door-integration.md

Filesystem layout: envctl is the path authority for meta installs under $META_ROOT/.local/{bin,lib,share,state,cache,tmp,opt}, with .toolchains only as compatibility. Odysseus data/log/cache must be envctl-managed and never committed; LifeOS Tauri FS stays scoped to app data/vault. [E1][O3][O4][L1][L2]
