...
| Backend | Role | LifeOS path | Gate |
|---|---|---|---|
| LifeOS provider commands | Current simple Claude/OpenAI/Gemini completion | Keep as fallback/basic assistant | Provider keyring/env and tests [L2] |
| Odysseus | Rich local AI workspace | `/ai` sandbox panel | envctl pin/auth/local bind/license/security [O1][O3][O4][O5] |
| weave | Coordination lanes, jobs, A2A, permissions | Background status/control bridge | token-light Damian/job-lane dispatch [W1][W2] |
| rvAgent / meta-ruvector | Rust-native agent runtime/substrate | future native agent runtime | selected trait/API/MCP proof first [V1][V2] |
| handoff | durable continuity agent state | notifications/resume/proof UI | ledger/status projection [H2][H3] |
