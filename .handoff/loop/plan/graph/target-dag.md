# Target DAG — LifeOS / Meta integration

```mermaid
flowchart LR
  prompt_hub -->|intent/prompt| rusty_idd[rusty-idd]
  rusty_idd -->|ready goal/spec| planning_engineer
  planning_engineer -->|build chunks| feature_forge
  feature_forge -->|install/verify| envctl
  envctl -->|component status/config| lifeos
  lifeos -->|/ai panel candidate| odysseus
  odysseus -->|MCP/API/events bridge| weave
  weave -->|jobs/messages/leases| handoff
  handoff -->|resume state| rusty_idd
  meta_ruvector -->|memory/vector/agent substrate| lifeos
  network_plane[network-control/lane/obscura] -->|fleet/network/privacy surfaces| lifeos
  network_plane -->|governed web egress| weave
```

Ordering rule: install and verification authority must flow through envctl before LifeOS treats a component as managed, while work-definition authority must flow through prompt_hub/rusty-idd before Feature Forge builds. [E1][R1][P2]
