...
1. Strict-upgrade only: no downgrade, destructive reset, or legacy removal until replacement is installed, configured, parity-proven, and rollback-safe. [H1][H3][E1]
2. Odysseus is AGPL and privileged; isolate by process/API/WebView/reverse proxy until license and security gates pass. [O4][O5]
3. Raw model/vector/search/notification services remain internal-only; LifeOS must not expose ChromaDB/SearXNG/ntfy/Ollama/vLLM/raw model ports. [O4]
4. Production code remains read-only during planning; permitted writes are planning artifacts and additive gate scripts. [H1]
5. Every roadmap item requires source citations; unverified claims remain out. [P2][H2]
