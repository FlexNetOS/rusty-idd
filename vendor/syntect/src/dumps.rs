...
///
/// The writer is encoded with the `postcard` crate and compressed with `flate2`.
#[cfg(feature = "dump-create")]
...
/// the `postcard` crate and then compressed with the `flate2` crate.
///
/// [`dump_to_writer`]: fn.dump_to_writer.html
#[cfg(feature = "dump-create")]
...
fn serialize_to_writer_impl<T: Serialize, W: Write>(
    ...
    // postcard is a non-streaming serializer: encode to a buffer, then write (optionally through
    // the zlib encoder). bincode's `serialize_into` streamed; buffering the whole dump is fine —
    // the largest asset is a few MB and dumps are produced once at build time.
    let encoded = postcard::to_allocvec(to_dump).map_err(|e| DumpError::Serialize(Box::new(e)))?;
    if use_compression {
        let mut encoder = ZlibEncoder::new(output, Compression::best());
        encoder.write_all(&encoded)?;
        // Explicitly finish so the trailing zlib bytes are written and surfaced as an error,
        // rather than relying on drop (which swallows IO errors).
        encoder.finish()?;
    } else {
...
fn deserialize_from_reader_impl<T: DeserializeOwned, R: BufRead>(
    ...
    // postcard deserializes from a byte slice, so read the (optionally decompressed) stream fully
    // into a buffer first. Mirrors the buffering in the serialize path above.
    let mut buf = Vec::new();
    if use_compression {
    ...
    postcard::from_bytes(&buf).map_err(|e| DumpError::Serialize(Box::new(e)))
}
...
impl SyntaxSet {
    ...
    /// the fact that SyntaxDefinitions are serializable with the postcard crate to cache dumps of
