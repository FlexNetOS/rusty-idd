...
plan-context:
    {{rusty_idd}} knowledge plan-context --workspace . --out .idd/knowledge/plan-context.json --change peer-architecture-detail-ingestion --goal "ingest bounded peer architecture graph details into Rusty IDD system planning"
    {{rusty_idd}} knowledge plan-context --workspace . --out .idd/knowledge/plan-context.md --change peer-architecture-detail-ingestion --goal "ingest bounded peer architecture graph details into Rusty IDD system planning"

plan-context-check:
    tmpdir=$(mktemp -d) && {{rusty_idd}} knowledge plan-context --workspace . --out "$tmpdir/plan-context.json" --change peer-architecture-detail-ingestion --goal "ingest bounded peer architecture graph details into Rusty IDD system planning" && {{rusty_idd}} knowledge plan-context --workspace . --out "$tmpdir/plan-context.md" --change peer-architecture-detail-ingestion --goal "ingest bounded peer architecture graph details into Rusty IDD system planning" && cmp -s .idd/knowledge/plan-context.json "$tmpdir/plan-context.json" && cmp -s .idd/knowledge/plan-context.md "$tmpdir/plan-context.md" || { echo ".idd/knowledge plan-context artifacts are stale; run just plan-context" >&2; rm -rf "$tmpdir"; exit 1; }; rm -rf "$tmpdir"
