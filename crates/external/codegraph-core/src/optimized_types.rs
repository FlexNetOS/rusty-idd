...
impl<T> AlignedVec<T> {
    pub fn new_aligned(capacity: usize, alignment: usize) -> Self {
        let alignment = alignment.max(mem::align_of::<T>());
        let layout = if capacity == 0 || mem::size_of::<T>() == 0 {
            None
        } else {
            Some(
                Layout::from_size_align(
                    mem::size_of::<T>()
                        ...
                        .expect("aligned vector capacity overflow"),
                    alignment,
                )
                .expect("valid aligned vector layout"),
            )
        };
        let ptr = if let Some(layout) = layout {
            // SAFETY: layout has non-zero size and valid alignment. The pointer is
            // managed by Drop and elements are initialized only through push().
            let raw = unsafe { alloc(layout) as *mut T };
            if raw.is_null() {
                handle_alloc_error(layout);
            }
            let ptr = NonNull::new(raw).expect("allocator returned non-null pointer");
    ...
    pub fn as_slice(&self) -> &[T] {
        // SAFETY: ptr is aligned and all indices below len are initialized.
        unsafe { std::slice::from_raw_parts(self.ptr.as_ptr(), self.len) }
    }

    pub fn as_mut_slice(&mut self) -> &mut [T] {
        // SAFETY: ptr is aligned, unique through &mut self, and all indices below
        // len are initialized.
    ...
    pub fn push(&mut self, value: T) -> Result<()> {
        if self.len < self.capacity {
            // SAFETY: len < capacity and this slot is uninitialized.
            unsafe {
    ...
    pub fn clear(&mut self) {
        // SAFETY: elements below len are initialized; each is dropped once.
        unsafe {
            for i in 0..self.len {
...
impl<T> Drop for AlignedVec<T> {
    fn drop(&mut self) {
        ...
        if let Some(layout) = self.layout {
            // SAFETY: ptr was allocated with this exact layout in new_aligned().
            unsafe {
...
mod tests {
    ...
    fn test_embedding_pool_efficiency() {
        ...
        assert_eq!(vec1.as_slice().as_ptr() as usize % vec1.alignment(), 0);
        pool.release(vec1);

        let vec2 = pool.acquire(); // Should reuse the previous vector
        assert_eq!(vec2.as_slice().as_ptr() as usize % vec2.alignment(), 0);
