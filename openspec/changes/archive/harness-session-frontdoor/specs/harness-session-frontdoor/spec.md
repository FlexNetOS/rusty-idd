...
Each vendor surface that supports a session-start hook SHALL invoke `rusty-idd
next` automatically when a session begins, so the computed next-step imperative
...
Rusty IDD SHALL detect duplicate ADR sequence numbers. `rusty-idd spec adr list
--check` SHALL exit non-zero when any duplicate ADR number exists that is not in
