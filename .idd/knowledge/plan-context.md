...
- Goal: Replace the always-loaded harness/tool overflow model with a Rusty IDD workflow-native, task-scoped Rust agent swarm architecture. After a goal is created, Rusty IDD routes each workflow stage, starting with scan, to a scoped Rust agent swarm that receives only the target, contracts, skills, hooks, helpers, tools, and evidence schema required for that exact stage. .codex/.claude/.kimi remain minimal runtime adapters. Rusty IDD owns workflow contracts, capability bundles, stage routing, validation, typed evidence handoff, and a general package-creation capability that creates or selects the right task-scoped package automatically instead of prompting ad hoc skill creation.
...
| `vox` | `main` | false | role:rust-code-surface |  |
